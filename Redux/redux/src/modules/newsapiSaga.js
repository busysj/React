...
// * 전체를 들고오는 기호
// as 별칭 별명 ( 전체를 들고올 때 어떤 이름으로 들고올 지)
// api.getNews()
import * as api from "../api/api";

// 액션타입
const GET_NEWS_START = "newsapiSaga/GET_NEWS_START"; // 실행할 사가액션의 이름
const GET_NEWS = "newsapiSaga/GET_NEWS"; // 로딩유무
const GET_NEWS_SUCCESS = "newsapiSaga/GET_NEWS_SUCCESS"; // 성공했을 때 값 할당
const GET_NEWS_FAILURE = "newsapiSaga/GET_NEWS_FAILURE"; // 실패했을 때 오류 할당

// 액션함수
export const getNewsStart = () => ({ type: GET_NEWS_START });

// 제너럴 함수 (saga에서 실행될 함수)
function* getNewsSaga() {
  ...
  try {
    ...
    // api 폴더와 파일 따로 만들어서 export 해서 사용 api.newsget()
  ...
  // 성공 : GET_NEWS_SUCCESS값 전달
  // 실패 : GET_NEWS_FAILURE로 오류 전달
}

// 작성한 제너럴 함수를 묶어주는 saga
...
// 리듀서값 작성
const newsapiSagaReducer = (state = initialState, action) => {
  switch (action.type) {
    case GET_NEWS:
      return {
        ...state,
